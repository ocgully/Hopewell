"""taskflow CLI — argparse over the library. v0.1 + v0.2 + v0.3 commands."""
...
from taskflow import __version__, SCHEMA_VERSION
from taskflow import attestation as att_mod
from taskflow import claim as claim_mod
...
from taskflow import evolve as evolve_mod
from taskflow import extensions as extensions_mod
from taskflow import flow_cli as flow_cli_mod
from taskflow import flow_trace_cli as flow_trace_cli_mod
from taskflow import hooks as hooks_mod
from taskflow import merge_driver as merge_driver_mod
from taskflow import network_cli as network_cli_mod
from taskflow import paths as paths_mod
from taskflow import reconciliation_cli as recon_cli_mod
from taskflow import release_cli as release_cli_mod
from taskflow import resume as resume_mod
...
from taskflow.project import CircularDependencyError
    ...
    if not args.quiet:
        ...
        if auto_backfill and ledger.is_file():
            try:
                ...
                pass
        print("  next steps:   `taskflow new --components work-item --title \"...\"`")
    return 0
...
def cmd_new(args) -> int:
    ...
    if args.format == "json":
        from taskflow.query import show
...
def cmd_list(args) -> int:
    project = _project(args)
    from taskflow.query import list_nodes
...
def cmd_ready(args) -> int:
    project = _project(args)
    from taskflow.query import ready
...
def cmd_link(args) -> int:
    ...
    try:
        ...
        print(f"taskflow: unknown edge kind '{args.kind}' — "
...
def cmd_graph(args) -> int:
    project = _project(args)
    from taskflow.render import views as views_mod
...
def cmd_claim(args) -> int:
    ...
    try:
        ...
        print(f"taskflow: git failed — {exc.stderr.strip() if exc.stderr else exc}", file=sys.stderr)
        return 2
    ...
    if args.format == "json":
        ...
        print(f"  next: start your work, then `taskflow close {c.node_id} ...` when done.")
    return 0
...
def cmd_merge_driver(args) -> int:
    # Invoked by git: `taskflow merge-driver jsonl <ancestor> <ours> <theirs>`
...
def cmd_uat(args) -> int:
    ...
    if args.action == "list":
        ...
        if args.format == "json":
            ...
            for r in rows:
                ...
                print(f"    pass:  taskflow uat pass  {r['id']} [--notes \"...\"]")
                print(f"    fail:  taskflow uat fail  {r['id']} --reason \"...\"")
                print(f"    waive: taskflow uat waive {r['id']} --reason \"...\"")
                print()
        return 0
    ...
    if args.action == "show":
        ...
        if not rows:
            print(f"taskflow: {args.id} has no needs-uat component", file=sys.stderr)
            return 1
        _print_json(rows[0])
        return 0
    ...
    print(f"taskflow: unknown uat action '{args.action}'", file=sys.stderr)
    return 1
...
def cmd_checkpoint(args) -> int:
    """Record a [next] checkpoint note on a node — `taskflow resume` surfaces it."""
    project = _project(args)
...
def cmd_evolve(args) -> int:
    ...
    if args.action == "add-node":
        ...
        if not components:
            print("taskflow: evolve add-node requires --components", file=sys.stderr)
            return 1
    ...
    if args.action == "add-loop":
        ...
        if not args.name or not over or not args.until:
            print("taskflow: evolve add-loop requires --name, --over, --until", file=sys.stderr)
            return 1
    ...
    print(f"taskflow: unknown evolve action '{args.action}'", file=sys.stderr)
    return 1
...
def cmd_migrate(args) -> int:
    ...
    if not args.quiet:
        ...
        print(f"  to bring newer TaskFlow project-level setup into an existing tree")
    return 0
...
def cmd_migrate_from_taskflow(args) -> int:
    ...
    # Walk up to find a project (either marker is fine — migrate.py keys on legacy).
    root = start
    while True:
        if (root / ".hopewell").is_dir() or (root / ".taskflow").is_dir():
            break
        if root.parent == root:
            root = start
            break
    ...
    if not args.quiet:
        if result["status"] == "migrated":
            ...
            print("Already on .taskflow/ — nothing to do.")
        elif result["status"] == "noop":
            print("No .hopewell/ directory found — nothing to migrate.")
...
def cmd_info(args) -> int:
    try:
        ...
        return 0
    from taskflow.query import graph
    _print_json({
        "version": __version__,
...
def cmd_query(args) -> int:
    project = _project(args)
    from taskflow import query as q
    if args.subject == "ready":
        ...
        print(f"taskflow: unknown query subject '{args.subject}'", file=sys.stderr)
        return 1
    _print_json(data)
    return 0
...
def cmd_agent(args) -> int:
    ...
    if args.action == "fingerprint":
        ...
        rec = reg.get(name)
        if rec is None:
            print(f"taskflow: no agent registered as {name!r}", file=sys.stderr)
            return 1
    ...
    print(f"taskflow: unknown agent action '{args.action}'", file=sys.stderr)
    return 1
...
def cmd_orch(args) -> int:
    ...
    if args.action == "status":
        ...
        return 0
    print(f"taskflow: unknown orch action '{args.action}'", file=sys.stderr)
    return 1
...
def cmd_github(args) -> int:
    ...
    if args.action == "config":
        ...
        return 0
    print(f"taskflow: unknown github action '{args.action}'", file=sys.stderr)
    return 1
...
def cmd_hooks(args) -> int:
    ...
    if full and minimal:
        print("taskflow hooks: --full and --minimal are mutually exclusive", file=sys.stderr)
        return 2
    if args.action == "install":
        ...
        if not args.quiet:
            ...
            if not full:
                print("(minimal install — run `taskflow hooks install --full` for gates)")
        if claude_code:
    ...
    if args.action == "test-pre-push":
        ...
        from taskflow import gates as gates_mod
        res = gates_mod.check_release_readiness(project.root,
                                                branch=getattr(args, "branch", None))
        print(res.format_for_hook())
        return 0 if res.ok else 1
    print(f"taskflow: unknown hooks action '{args.action}'", file=sys.stderr)
    return 1
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # init
    sp = sub.add_parser("init", help="Initialise .taskflow/ in the project")
    sp.add_argument("--prefix", default="TF",
                    help="ID prefix for new ticket numbers (default: TF). "
                         "Pass --prefix HW for projects continuing the legacy scheme.")
    ...
    # render
    sp = sub.add_parser("render", help="Regenerate .taskflow/views/*")
    sp.add_argument("--quiet", action="store_true")
    ...
    sp = sub.add_parser("migrate", help="Re-apply idempotent setup after a TaskFlow upgrade")
    sp.add_argument("--quiet", action="store_true")
    sp.set_defaults(func=cmd_migrate)

    sp = sub.add_parser("migrate-from-hopewell",
                        help="Rename legacy .hopewell/ to .taskflow/ and rewrite internal references")
    sp.add_argument("--dry-run", action="store_true",
    ...
    np = nsub.add_parser("init", help="Scaffold .taskflow/network/")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        from taskflow.meta import HopewellVersionError
...
def main_deprecated_alias(argv=None) -> int:
    """Deprecation-shim entry point for the legacy `hopewell` script.
    ...
    """
    sys.stderr.write(
        "hopewell: 'hopewell' is the legacy name for taskflow; "

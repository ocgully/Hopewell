...
from taskflow import network_defaults as defaults_mod
from taskflow import paths as paths_mod
from taskflow.executor import Executor, Route, validate_executor_id
...
def cmd_network_executor(args) -> int:
    ...
    if action == "add":
        if not args.id:
            print("taskflow: executor add requires an id", file=sys.stderr)
            return 1
        try:
        ...
        if not components:
            print("taskflow: executor add requires --components c1,c2,...", file=sys.stderr)
            return 1
        component_data = {}
        if args.component_data:
            try:
                ...
                print(f"taskflow: --component-data is not valid JSON: {e}", file=sys.stderr)
                return 1
            if not isinstance(component_data, dict):
                print("taskflow: --component-data must be a JSON object", file=sys.stderr)
                return 1
        ex = Executor(
            id=args.id,
    ...
    if action == "rm":
        if not args.id:
            print("taskflow: executor rm requires an id", file=sys.stderr)
            return 1
        ok = net_mod.remove_executor(root, args.id)
        if not ok:
            print(f"taskflow: no executor named {args.id!r}", file=sys.stderr)
            return 1
        if args.format == "json":
    ...
    if action == "show":
        if not args.id:
            print("taskflow: executor show requires an id", file=sys.stderr)
            return 1
    ...
    print(f"taskflow: unknown executor action {action!r}", file=sys.stderr)
    return 1
...
def cmd_network_route(args) -> int:
    ...
    if action == "add":
        if not args.from_id or not args.to:
            print("taskflow: route add requires <from> <to>", file=sys.stderr)
            return 1
        r = Route(
    ...
    if action == "rm":
        if not args.from_id or not args.to:
            print("taskflow: route rm requires <from> <to>", file=sys.stderr)
            return 1
    ...
    print(f"taskflow: unknown route action {action!r}", file=sys.stderr)
    return 1

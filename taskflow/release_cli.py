...
from taskflow import release_confidence as rc_mod
...
def cmd_release_start(args) -> int:
    ...
    try:
        ...
        print(f"taskflow release: {e}", file=sys.stderr)
        return 1
...
def cmd_release_scope(args) -> int:
    ...
    try:
        ...
        print(f"taskflow release scope: {e}", file=sys.stderr)
        return 1
    if _fmt(args) == "json":
...
def cmd_release_report(args) -> int:
    ...
    try:
        ...
        print(f"taskflow release report: {e}", file=sys.stderr)
        return 1
    if _fmt(args) == "json":
...
def cmd_release_score(args) -> int:
    ...
    try:
        ...
        print(f"taskflow release score: {e}", file=sys.stderr)
        return 1
    if _fmt(args) == "json":
        _print_json(sc)
        return 0
...
def cmd_release_finalize(args) -> int:
    ...
    try:
        ...
        print(f"taskflow release finalize: {e}", file=sys.stderr)
        return 1
    if _fmt(args) == "json":
        _print_json(res)
    else:
...
def cmd_release_kickback(args) -> int:
    ...
    if not affected:
        print("taskflow release kickback: --affected must list at least one node",
              file=sys.stderr)
        return 1
    try:
        ...
        print(f"taskflow release kickback: {e}", file=sys.stderr)
        return 1
    if _fmt(args) == "json":
        _print_json(res)
    else:
...
def cmd_release_show(args) -> int:
    ...
    if node is None:
        print(f"taskflow release show: no release for {args.version!r}",
              file=sys.stderr)
        return 1
    block = (node.component_data or {}).get(release_mod.COMPONENT) or {}
    summary = {

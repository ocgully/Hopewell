...
`needs-uat` component captures the gap:
  ...
  - `taskflow uat flag <id>` adds the component (usually at creation time)
  - `taskflow uat list` shows every flagged node whose UAT status is pending
  - `taskflow uat {pass|fail|waive} <id>` records the outcome
  - `taskflow uat backfill` retroactively flags older done nodes when a
